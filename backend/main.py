...
class SignupRequest(BaseModel):
    full_name: str = Field(..., example="Chris Tucker")
    email: EmailStr = Field(..., example="chris@example.com")
...
def sign_up(input:SignupRequest):
    try:
        ...
        db.execute(query, {"name": input.full_name, "email": input.email, "password": hashedPassword})
        db.commit()

        return {"message": "User Created Successfully",
                "data": {"name": input.full_name, "email":input.email}}
